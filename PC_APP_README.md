...
Paper RoomではURLだけでなく、PC上のPDF/TXT/Markdownを直接読み込めます。

- `ファイルを選ぶ`: 1本の論文ファイルを選択
- `フォルダを開く`: フォルダ内の対応ファイルを一覧し、読みたい1本を選択

PDFは本文テキストを抽出してTrainerに保存します。スキャン画像だけのPDFなど、コピー可能な本文が含まれないPDFは読み取れない場合があります。

## Paper Roomのタグとメモ

Paper Roomでは、論文追加時にカンマ区切りでタグを入れられます。Paper CardをCodexで生成すると、Abstract下のKeywordsに近い分野タグも自動で保存され、一覧上部のタグから論文を絞り込めます。

本文・補足メモでは、ツールボタンまたは `Cmd/Ctrl+B`, `Cmd/Ctrl+I`, `Cmd/Ctrl+U` で太字、イタリック、下線を入れられます。
